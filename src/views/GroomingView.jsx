...
export default function GroomingView({ onReadArticle, onNavigate }) {
  const [page, setPage] = useState(1);

  const groomingArticles = articleData.filter(a => a.tag === 'Grooming');
  const totalPages = Math.ceil(groomingArticles.length / ARTICLES_PER_PAGE);
  const paginatedArticles = groomingArticles.slice((page - 1) * ARTICLES_PER_PAGE, page * ARTICLES_PER_PAGE);

  const featuredArticle = groomingArticles[0];
  const moreArticles = groomingArticles.filter(a => a.id !== featuredArticle?.id).slice(0, 3);
  ...
  return (
    <main className="fix">
          ...
          <div className="row align-items-center">
            <div className="col-lg-8">
              ...
              <h1 style={{ fontSize: '40px', fontWeight: '900', color: '#1a1a1a', lineHeight: '1.2', marginBottom: '16px' }}>
                The Double Coat Guide<br />Nobody Told You About
              </h1>
              <p style={{ fontSize: '16px', color: '#555', lineHeight: '1.7', maxWidth: '560px', margin: 0 }}>
                Shaving is a myth. Over-bathing wrecks their skin. Learn the real science behind caring for a Labrador's waterproof double coat in Sri Lanka's tropical climate.
              </p>
            </div>
            <div className="col-lg-4 d-none d-lg-flex justify-content-end align-items-center">
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '48px', fontWeight: '900', color: '#E2001A', lineHeight: 1 }}>{groomingArticles.length}</div>
      ...
      {/* Grooming Quick Rules */}
      <section style={{ background: '#1a1a1a', padding: '28px 0' }}>
        <div className="container">
          <div className="row g-3 text-center">
            {[
              { icon: 'fa-shower', text: 'Every 4–6 weeks only', label: 'Bath Frequency' },
              { icon: 'fa-cut', text: 'Never shave the coat', label: 'Golden Rule' },
              { icon: 'fa-calendar-check', text: 'Trim nails every 3–4 weeks', label: 'Nail Care' },
            ].map((fact, i) => (
              <div className="col-md-4" key={i}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
                  ...
                  <div style={{ textAlign: 'left' }}>
                    ...
                    <div style={{ fontSize: '13px', fontWeight: '700', color: '#fff' }}>{fact.text}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <section style={{ background: '#fff', paddingTop: '60px', paddingBottom: '0' }}>
          <div className="container">
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '28px' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: '800', color: '#1a1a1a', textTransform: 'uppercase', letterSpacing: '1.5px' }}>Top Grooming Guide</span>
            </div>
            <div
      ...
      {/* Coat Care & Skin Science */}
      {moreArticles.length > 0 && (
        <section style={{ background: '#fff', paddingTop: '60px', paddingBottom: '20px' }}>
          <div className="container">
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '28px' }}>
              <div style={{ width: '4px', height: '22px', background: '#E2001A', borderRadius: '2px' }}></div>
              <span style={{ fontSize: '13px', fontWeight: '800', color: '#1a1a1a', textTransform: 'uppercase', letterSpacing: '1.5px' }}>Coat & Skin Science</span>
            </div>
      ...
      {/* Salmon Oil Callout — subtle Beaphar mention */}
        ...
        <div className="container">
          <div style={{ border: '1px solid #D4AF37', borderRadius: '16px', background: '#fff', padding: '32px 36px', display: 'flex', gap: '16px' }}>
            <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: '#FFF2F2', border: '1px solid #FFD1D1', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <i className="fas fa-water" style={{ fontSize: '18px', color: '#E2001A' }}></i>
            </div>
            <div>
              <p style={{ fontSize: '15px', fontWeight: '800', color: '#1a1a1a', margin: '0 0 8px 0' }}>
                The real fix for shedding is inside the bowl, not outside.
              </p>
              <p style={{ fontSize: '13px', color: '#666', margin: 0, lineHeight: '1.7' }}>
                Excessive shedding is almost always a skin barrier issue. Cold-pressed salmon oil delivers Omega-3 and Omega-6 fatty acids that strengthen the intercellular lipid cement in skin cells — locking hair follicles in and reducing loose hair dramatically. Pair with an undercoat rake twice weekly for maximum results.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* All Grooming Articles — Paginated */}
      <section style={{ background: '#fff', paddingTop: '60px', paddingBottom: '80px' }}>
        <div className="container">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px', flexWrap: 'wrap', gap: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div style={{ width: '4px', height: '22px', background: '#E2001A', borderRadius: '2px' }}></div>
              <span style={{ fontSize: '13px', fontWeight: '800', color: '#1a1a1a', textTransform: 'uppercase', letterSpacing: '1.5px' }}>All Grooming Guides</span>
            </div>
            <span style={{ fontSize: '13px', color: '#888' }}>{groomingArticles.length} articles total</span>
          </div>
